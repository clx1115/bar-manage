  ...
  <el-form ref="formRef" size="default" label-width="100px" class="form-container">
    <el-row :gutter="35">
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
        <el-form-item label="兑换数量" prop="bidPrice">
          <el-input v-model="formData.convertAmount" placeholder="个人可兑换数量" clearable class="input200"></el-input>
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
        <el-form-item label="商品类型" prop="isVirtual">
          <el-radio-group v-model="formData.isVirtual" @click="changeMode()">
            <el-radio :label="0">实物</el-radio>
            <el-radio :label="1">虚拟</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20" v-if="formData.isVirtual == 1">
        <el-form-item>
          <el-button @click="onOpenBind()" size="small" text type="primary">绑定优惠劵</el-button>
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20" v-if="useRules.list.length > 0">
        <el-form-item>
          <el-table :data="useRules.list">
            <el-table-column width="55" type="index" label="序号" />
            <el-table-column prop="couponId" label="ID" width="120" />
            <el-table-column prop="couponTitle" label="名称" width="120" show-overflow-tooltip />
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20" v-if="formData.isVirtual == 0">
      ...
      <template v-if="formData.attrType === 1">
        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
          <el-form-item label="兑换积分" prop="score">
            <el-input v-model="formData.simpleProudct.score" placeholder="兑换所需积分" clearable class="input200"></el-input>
          </el-form-item>
        </el-col>
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
                ...
                <div class="list">
                  <draggable v-model="item.list" @start="drag = true" @end="drag = false" item-key="index">
                    <template #item="{ element, index }">
                      <el-tag class="item" size="large" closable @close="onDelAttrValue(attrIndex, index)">{{ element
                      }}</el-tag>
                    </template>
                  </draggable>
          ...
          <el-form-item>
            <el-table :data="formData.skuList" height="300">
              ...
              <el-table-column v-if="formData.priceModel === 1 || formData.priceModel === 3" align="center" width="150" style="display: none;">
                ...
                <template #default="{ row }">
                  <el-input v-model="row.score" placeholder="所需积分" size="small" />
                </template>
              </el-table-column>
    ...
    <selectDialog ref="selectDialogRef" :type="formData.isVirtual" @select="bindProduct" />
  </el-form>
</template>

<script setup lang="ts">
import { reactive, toRefs, ref, watch, computed, onMounted, defineAsyncComponent } from 'vue'
...
const bindProduct = (val: any) => {
  ...
  console.log(111, state.useRules.list)
}
...
const initFormData = (val: any) => {
  ...
  state.formData.isVirtual = detailData?.isVirtual
  state.formData.convertAmount = detailData?.convertAmount
  state.formData.attrType = skuData.attrType
  state.formData.priceModel = skuData.priceModel
  state.useRules = detailData?.coupon
  ...
  if (skuData.attrType === 1) {
    ...
    if (skuData.priceModel === 3) {
      state.formData.stepPrice = skuData.stepPrice
    }
...
//子传父
const transmit = () => {
  state.formData.coupon = JSON.stringify(state.useRules)
