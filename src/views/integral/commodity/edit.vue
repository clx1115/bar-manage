    ...
    <el-card shadow="hover" class="layout-padding-auto">
      <el-tabs v-model="activeName">
        <el-tab-pane label="商品信息" name="info">
          <product-info ref="infoRef" :detailData="detailData" />
        </el-tab-pane>
...
import { getScoreDetail, updateScore } from '@/api/integral/index'
...
// 获取详情
const getDetailData = () => {
  const scoreProductId = router.currentRoute.value.query.scoreProductId
  getScoreDetail({
    id: scoreProductId,
  }).then((res) => {
    state.detailData = {
      id: res.id,
      isVirtual: Number(res.isVirtual),
      categoryId: res.categoryId,
      name: res.name,
      unit: res.unit,
      convertAmount: res.convertAmount,
      stock: res.stock,
      sketch: res.sketch,
      ...
      sort: res.sort,
      coupon: res.coupon ? JSON.parse(res.coupon) : {},
...
const onSubmit = () => {
  ...
  if (formData.attrType === 1) {
    console.log(123, formData.simpleProudct)
    ...
    formData.maxScore = formData.minScore
  } else {
  ...
  formData.id = router.currentRoute.value.query.scoreProductId
