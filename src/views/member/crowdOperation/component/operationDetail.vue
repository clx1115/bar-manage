    ...
    <el-form ref="formRef" :model="ruleForm" :rules="rules" size="default" label-width="120px"
      ...
      <el-row>
        <el-col :xs="24" :sm="12" :md="9" :lg="9" :xl="9">
          <el-form-item label="计划人群" prop="groupList">
            <el-button type="primary" text @click="selectGroup" v-if="planType == '自定义人群'">
              选择人群
            </el-button>
            <el-button type="primary" text @click="addCustomPhone" v-else-if="planType == '指定手机号客户'">
              添加手机号
            </el-button>
            <el-table :data="groupList" header-cell-class-name="header-name" v-else>
              <el-table-column label="人群" show-overflow-tooltip min-width="100">
                <template>
                  {{ planType }}
                </template>
              </el-table-column>
              <el-table-column prop="num" label="预计覆盖人数" show-overflow-tooltip min-width="100">
                <template #default="{ row }">
                  {{ row.num }}
                </template>
              </el-table-column>
            </el-table>
            <div class="info">每次推送人数建议少于10万，否则容易出现延时的情况</div>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row v-if="planType == '加购未支付客户'">
        <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
          <el-form-item label="触发条件" prop="triggerCondition">
            <div style="display: flex;">
              在客户有加入购物车行为但一直没有购买的第
              <el-input v-model="ruleForm.triggerCondition" style="width: 100px;margin: 0 5px"></el-input>
              天进行定向营销
            </div>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col :xs="24" :sm="16" :md="16" :lg="16" :xl="16">
          <el-form-item label="计划方式" prop="planMethod">
            <el-radio-group v-model="ruleForm.planMethod">
              <el-radio :label="1">自动长期计划</el-radio>
            </el-radio-group>
            <div class="info ml-2">人群中如有满足条件的新增客户，新增的客户会自动收到营销通知。长期运营计划剩余可创建20条</div>
            <el-form-item label="计划执行时间" prop="planTimeRange" v-if="ruleForm.planMethod == 1">
              <el-date-picker v-model="ruleForm.planTimeRange" type="daterange" value-format="x" style="flex-grow: 0;"
                start-placeholder="开始日期" end-placeholder="结束日期" />
              <div class="info">在指定时间，给达到条件的客户直接将优惠券发放到账；每次发放权益时会通知消费者，当消息通知达到限制次数后不再通知</div>
            </el-form-item>
            <el-form-item label="发放规则" prop="planGiveRule" v-if="ruleForm.planMethod == 1">
              <el-radio-group v-model="ruleForm.planGiveRule">
                <el-radio :label="1" style="margin-bottom: 8px;">一次</el-radio>
                <el-radio :label="2" style="margin-bottom: 8px;">
                  活动期内，每自然月
                  <el-input v-model.number="ruleForm.planGiveRuleVal1" :disabled="ruleForm.planGiveRule !== 2"
                    style="width: 100px;"></el-input>
                  号，发放权益一次
                </el-radio>
                <el-radio :label="3" style="margin-bottom: 8px;">
                  活动期内，每
                  <el-select v-model="ruleForm.planGiveRuleVal2" :disabled="ruleForm.planGiveRule !== 3"
                    style="width: 100px;">
                    <el-option v-for="(val, key) in week" :key="key" :label="val.label" :value="val.value">
                    </el-option>
                  </el-select>
                  发放权益一次
                </el-radio>
                <el-radio :label="4">
                  活动期内，每
                  <el-input v-model.number="ruleForm.planGiveRuleVal3" :disabled="ruleForm.planGiveRule !== 4"
                    style="width: 100px;"></el-input>
                  天，发放权益一次
                </el-radio>
              </el-radio-group>
              <div class="info ml-2">第一次发权益时间为创建计划后的第n天</div>
            </el-form-item>
            <el-radio-group v-model="ruleForm.planMethod">
              <el-radio :label="2">手动定时计划</el-radio>
            </el-radio-group>
            <div class="info ml-2">在指定时间向计划人群发送营销通知</div>
            <el-form-item label="计划执行时间" prop="planTimeType" v-if="ruleForm.planMethod == 2">
              <el-radio-group v-model="ruleForm.planTimeType">
                <el-radio :label="1">立即执行</el-radio>
                <el-radio :label="2">
                  定时执行
                  <el-date-picker v-model="ruleForm.planTime" type="date" value-format="x" style="flex-grow: 0;"
                    placeholder="请选择日期" :disabled="ruleForm.planTimeType == 1" />
                </el-radio>
              </el-radio-group>
            </el-form-item>
          </el-form-item>
      ...
      <el-row>
        <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
          <el-form-item label="优惠券配置" prop="couponSet">
            <el-checkbox-group v-model="ruleForm.couponSet">
              <el-checkbox :label="1">发放优惠券</el-checkbox>
            </el-checkbox-group>
            <div class="info ml-2">发放优惠券能有效刺激转化（优惠券将直接发放给客户，不需要领取）</div>
            <el-button type="primary" text @click="selectCoupon"
              v-if="ruleForm.couponSet && ruleForm.couponSet.indexOf(1) > -1">
              选择优惠券
            </el-button>
          </el-form-item>
        </el-col>
      ...
      <div class="tab-title" v-if="ruleForm.planMethod == 1">时间设置</div>
      <el-row v-if="ruleForm.planMethod == 1">
        <el-col :xs="24" :sm="24" :md="16" :lg="16" :xl="16">
          <el-form-item label="发放权益、通知时间" prop="grantTime" label-width="150px">
            <div style="display: flex">
              每天
            ...
            <div class="info ml-2">为避免骚扰用户，限制通知时间范围07:00~19:00；同时设置了发放权益和通知时，会先给客户发放权益礼包，再给发放成功的客户发通知</div>
          </el-form-item>
        </el-col>
      </el-row>
...
const phoneDialog = defineAsyncComponent(
  () => import('./phoneDialog.vue')
)
...
const { loading, ruleForm, rules, submitTxt, week, channelFlag, channel, planType, groupList } = toRefs(state);
