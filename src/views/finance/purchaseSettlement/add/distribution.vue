<template>
  <div class="card">
    ...
    <div class="search">
      <el-form :inline="true" :model="searchForm" label-width="auto">
        <el-form-item label="制单日期：" prop="date">
          <el-date-picker v-model="searchForm.date" type="datetimerange" :shortcuts="shortcuts" range-separator="-" />
        </el-form-item>
    ...
    <el-table :data="list1" style="width: 100%; margin-top: 10px" :header-cell-style="{ background: '#f5f7fa' }" @selection-change="selectionList1 = $event">
      ...
      <el-table-column prop="memberNo" label="对账状态" min-width="150" />
  ...
  <div class="bottom">
    <div>
      已选采购明细：<span style="margin-right: 4px">{{ selectionList1.length }}</span>已选费用单：<span>{{ selectionList2.length }}</span>
    </div>
