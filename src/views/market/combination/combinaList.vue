  ...
  <div class="page-container layout-padding">
    <el-card shadow="hover" class="layout-padding-auto">
      ...
      <el-table :data="list" v-loading="loading" style="width: 100%">
        <el-table-column prop="coverImgUrl" label="头像" width="100">
          <template #default="{ row }"><img :src="row.coverImgUrl" class="smallImage"></template>
        </el-table-column>
        <el-table-column prop="name" label="开团团长" min-width="100" show-overflow-tooltip />
        <el-table-column label="开团时间" width="80">
          <template #default="{ row }">
            {{ parseMoney(row.oldPrice) }}
          </template>
        </el-table-column>
        <el-table-column label="拼团商品" width="150">
          <template #default="{ row }">
        ...
        <el-table-column label="几人团" width="80">
          <template #default="{ row }">
            {{ parseMoney(row.oldPrice) }}
          </template>
        </el-table-column>
        <el-table-column label="几人参加" width="80">
          <template #default="{ row }">
            {{ parseMoney(row.oldPrice) }}
          </template>
        </el-table-column>
        <el-table-column label="结束时间" width="80">
          <template #default="{ row }">
            {{ parseMoney(row.oldPrice) }}
          </template>
        </el-table-column>
