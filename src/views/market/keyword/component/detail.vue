      ...
      <el-form ref="detailFormRef" :model="ruleForm" :rules="rules" size="default" label-width="90px">
        <el-row :gutter="35">
          <el-col :xs="24" :sm="12" :md="12" :lg="15" :xl="12" class="mb20">
            <el-form-item label="搜索页位置" prop="from">
...
const openDialog = (e: any, id: any) => {
  ...
  if (!id) {
    ...
    KeywordDetail({ id }).then((data) => {
      state.ruleForm = {
        id: data.searchKeyword.id,
        source: data.searchKeyword.source,
        keyword: data.searchKeyword.keyword,
        sort: data.searchKeyword.sort,
      }
    })
  }
